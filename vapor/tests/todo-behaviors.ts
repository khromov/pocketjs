// vapor/tests/todo-behaviors.ts — the VAPOR TODO semantics, pinned once.
//
// oracle.test.ts runs these on real Vue Vapor over todo.tsx and
// svelte-oracle.test.ts runs them on real Svelte over todo.svelte. Both
// oracles paint the same 30x20 grid, so the expected strings are shared.

import { describe, expect, test } from "bun:test";
import { Button } from "../host/input.ts";
import type { Oracle } from "../oracle/boot.ts";

export function todoBehaviors(name: string, boot: () => Promise<Oracle>): void {
  const line = (oracle: Oracle, y: number): string => oracle.grid().chars[y];
