...
import { TAU, angleTo, due, fireAimed, fireFan, fireRing, fireSpiral } from "./patterns.ts";
...
export function updateBoss(g: Game, dt: number): void {
  ...
  switch (b.phase) {
    case 0:
      ...
      if (due(t, dt, 48)) {
        b.angle += 0.15;
        fireRing(eb, b.x, b.y, 20, 1.3, b.angle, KIND_BLUE);
      }
      if (due(t, dt, 24, 12)) {
        fireFan(eb, b.x, b.y, angleTo(b.x, b.y, g.px, g.py), 3, 0.3, 2.2, KIND_RED);
      }
      break;
    case 1:
      ...
      if (due(t, dt, 4)) {
        b.angle += 0.11;
        fireSpiral(eb, b.x, b.y, b.angle, 2, 1.9, KIND_GREEN);
      }
      if (due(t, dt, 90)) {
        b.angle2 += 0.3;
        fireRing(eb, b.x, b.y, 16, 1.0, b.angle2, KIND_BLUE);
      }
      break;
    case 2:
      ...
      if (!armed) break;
      if (due(t, dt, 60)) {
        fireFan(eb, b.x, b.y, g.rng.float() * TAU, 7, 1.2, 2.4, KIND_RED);
      }
      if (due(t, dt, 60, 20)) {
        fireFan(eb, b.x, b.y, angleTo(b.x, b.y, g.px, g.py), 5, 0.8, 1.6, KIND_RED);
      }
      if (due(t, dt, 120, 60)) {
        b.angle += 0.25;
        b.angle2 -= 0.25;
        fireRing(eb, b.x, b.y, 12, 1.1, b.angle, KIND_BLUE);
        fireRing(eb, b.x, b.y, 12, 1.1, b.angle2, KIND_GREEN);
      }
      break;
    default:
      ...
      if (due(t, dt, 36)) {
        b.angle += 0.2;
        b.angle2 -= 0.2;
        fireRing(eb, b.x, b.y, 18, 1.4, b.angle, KIND_BLUE);
        fireRing(eb, b.x, b.y, 18, 1.2, b.angle2, KIND_RED);
      }
      if (due(t, dt, 12)) fireAimed(eb, b.x, b.y, g.px, g.py, 2.8, KIND_GREEN);
      break;
  }
}
