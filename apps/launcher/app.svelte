<script lang="ts">
  // apps/launcher/app.svelte — the Pocket Launcher: Cover Flow over every
  ...
  import { registerTexture } from "@pocketjs/framework/svelte";
  import { Image, Text, View, type NodeMirror } from "@pocketjs/framework/svelte/components";
  import { animate, createJumpBatch } from "@pocketjs/framework/svelte/animation";
  import { BTN, touches } from "@pocketjs/framework/svelte/input";
  import { onButtonPress, onFrame } from "@pocketjs/framework/svelte/lifecycle";
  import { appTable, frozenShot, launchApp } from "@pocketjs/framework/svelte/launcher";
  ...
  let viewport = $state(initialViewport);
  const cardLeft = $derived(viewport.w / 2 - 96);
  const cardTop = $derived(Math.max(24, Math.round((viewport.h - 76 - 218) / 2)));
  ...
  let sel = $state(resumeIndex >= 0 ? resumeIndex : 0);
  const cardEls: (NodeMirror | undefined)[] = new Array(apps.length);
  // The deck's opening pose, sampled once: after mount the four card props
  // belong to animate()/jump(), so the style prop must never re-derive them
  // from sel.
  const initialTargets = apps.map((_, i) => targetFor(i - sel));
  ...
  const selected = $derived(apps[sel]);
</script>
...
<View debugName="LauncherScreen" class="relative w-full h-full bg-[#05060a] overflow-hidden">
  ...
  {#if apps.length > 0}
    <View debugName="Deck" class="absolute inset-0 perspective-[620]">
      {#each apps as app, i (app.output)}
        ...
        <View
          nodeRef={(el: NodeMirror) => (cardEls[i] = el)}
          debugName="Card"
          class="absolute w-[192] h-[218]"
          style={{
            insetL: cardLeft,
            insetT: cardTop,
            translateX: initialTargets[i].translateX,
            translateZ: initialTargets[i].translateZ,
            rotateY: initialTargets[i].rotateY,
            opacity: initialTargets[i].opacity,
          }}
        >
    ...
    >
      ...
      <Text class="text-xs text-slate-500">{`${sel + 1} / ${apps.length} · ${selected.id}`}</Text>
      {#if resume && selected.output === resume}
  ...
  {:else}
    ...
    >No apps embedded — build with tools/launcher.ts</Text>
  {/if}

  <Text class="absolute left-0 right-0 bottom-2 text-center text-xs text-slate-600">{table
      ? resume
        ? "L/R · CIRCLE launch · CROSS back · touch sides/center"
        : "L/R · CIRCLE launch · touch sides/center"
      : "browse only — this host cannot switch apps"}</Text>
</View>
