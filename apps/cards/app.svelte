<!-- apps/cards/app.svelte — "card carousel" showcase: three feature cards in a
     row, LEFT/RIGHT d-pad moves focus (native focus: variants lift + brighten
     the focused card — zero JS on focus change), CIRCLE flips a keyed detail
     panel that springs up into place without color fades. Two slow gradient
     streaks drift behind everything (long native tweens started once on mount —
     deterministic fixed-dt, no per-frame JS).
...
<script lang="ts">
  ...
  let open = $state(-1);
  const selected = $derived(open >= 0 ? CARDS[open] : undefined);
  ...
  // The detail panel remounts (keyed block) per card, so this runs again on
  // every open and the translate-y spring replays; colors stay static. The
  // spring is declared here rather than in `nodeRef` because `nodeRef` fires
  // before the `style` prop lands, and the panel has to start at 22.
  let detail: NodeMirror | undefined = $state(undefined);
  $effect(() => {
    if (detail) spring(detail, "translateY", 0);
  });
</script>

{#snippet detailPanel(card: Card)}
  <View
    debugName="Detail"
    style={{ translateY: 22 }}
    class="flex-row items-center gap-3 p-3 rounded-xl shadow-md bg-white border-slate-200"
    nodeRef={(node: NodeMirror) => (detail = node)}
  >
    <View class={card.bar} />
...
>
  ...
  <View debugName="CardRow" class="flex-row gap-3">
    {#each CARDS as card, i (card.title)}
      <View class={card.cls} focusable onPress={() => (open = open === i ? -1 : i)}>
