...
Solid is the default framework. Vue Vapor, Octane and Svelte build beside the
Solid artifacts by adding a suffix:
...
`framework/src/components-vue-vapor.ts` for Vue Vapor,
`framework/src/components-octane.tsx` for Octane, and
`framework/src/components-svelte.ts` for Svelte. **One row per public subpath in
