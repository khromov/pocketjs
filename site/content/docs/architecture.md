...
with no DOM shim; Svelte compiles `.svelte` components against its
custom-renderer API, with `renderer-svelte.ts` as the renderer object. The rendering, layout, styling, animation, and text engine is
...
Svelte compiles `.svelte` components with `experimental.customRenderer`
pointed at `renderer-svelte.ts`.
...
state. The Solid/Vue/Octane/Svelte adapters keep a lightweight JS *mirror* of
the tree —
