...
Solid entries pass a closure (`mount(() => <App />)`); Vue Vapor, Octane and
Svelte entries pass the component itself (`mount(App)`). In Octane, JSX inside a
...
not need target checks. Solid, Vue Vapor, Octane and Svelte expose the same
functions from their lifecycle subpaths.
