...
Generated by `bun tools/bundle-size.ts` at 2026-09-06T04:47:40.548Z from git `edbca53` **with a dirty working tree** — these numbers are not reproducible from the committed tree alone.
...
Over 6 demos, Svelte is **2.15x** Solid as shipped and **1.87x** gzipped — **larger than Solid** on both.

Over 3 demos, Svelte is **0.96x** Vue Vapor as shipped and **0.68x** gzipped. The two disagree: as shipped Svelte is about the same, gzipped it is smaller. Vue Vapor vendors a pre-minified runtime and Svelte vendors raw source, so the raw ratio is comparing packaging; the gzip ratio is the one that reflects the runtimes.

Over 2 demos, Svelte is **0.82x** Octane as shipped and **0.89x** gzipped — **smaller than Octane** on both.
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `cards` | 336.1 KiB | 286.7 KiB | **0.85x** | 157.3 KiB | 116.4 KiB | 44.4 KiB | 40.8 KiB | **0.92x** |
| `hero` | 339.5 KiB | 270.8 KiB | **0.80x** | 157.4 KiB | 109.6 KiB | 44.6 KiB | 38.5 KiB | **0.86x** |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `cards` | 281.7 KiB | 286.7 KiB | **1.02x** | 158.2 KiB | 116.4 KiB | 56.7 KiB | 40.8 KiB | **0.72x** |
| `hero` | 308.2 KiB | 270.8 KiB | **0.88x** | 166.9 KiB | 109.6 KiB | 60.1 KiB | 38.5 KiB | **0.64x** |
| `lab (vue-sfc-lab ↔ svelte-lab)` | 301.2 KiB | 293.9 KiB | **0.98x** | 169.9 KiB | 116.8 KiB | 60.9 KiB | 40.8 KiB | **0.67x** |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `cards` | 129.1 KiB | 286.7 KiB | **2.22x** | 60.0 KiB | 116.4 KiB | 21.1 KiB | 40.8 KiB | **1.93x** |
| `chrome` | 122.9 KiB | 265.7 KiB | **2.16x** | 56.3 KiB | 107.5 KiB | 19.9 KiB | 37.8 KiB | **1.90x** |
| `cursor` | 120.6 KiB | 275.5 KiB | **2.28x** | 55.0 KiB | 111.0 KiB | 19.8 KiB | 39.5 KiB | **1.99x** |
| `hero` | 133.1 KiB | 270.8 KiB | **2.03x** | 61.6 KiB | 109.6 KiB | 21.3 KiB | 38.5 KiB | **1.80x** |
| `launcher` | 138.5 KiB | 284.2 KiB | **2.05x** | 64.5 KiB | 117.1 KiB | 22.7 KiB | 41.3 KiB | **1.81x** |
| `motions` | 172.5 KiB | 369.7 KiB | **2.14x** | 90.6 KiB | 155.9 KiB | 25.9 KiB | 45.7 KiB | **1.76x** |
...
| Svelte | **119.1 KiB** | 69.1 KiB | 2.2 KiB | 7.8 KiB | 2.1 KiB | 200.3 KiB |

The framework runtime alone, from the same builds:

| Framework | npm package | Runtime, normalised | Relative to Svelte |
| --- | --- | ---: | ---: |
| Solid | `solid-js` | 21.6 KiB | 0.18x |
| Vue Vapor | `vue` | 112.9 KiB | 0.95x |
| Octane | `octane` | 163.2 KiB | 1.37x |
| Svelte | `svelte` | 119.1 KiB | 1.00x |
