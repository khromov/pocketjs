...
/** Where the `.MIPS.abiflags` wrappers live. Beside the SDK and cargo-psp
 *  tools, so one `POCKET_STACK_CACHE_DIR` moves every PSP artifact. */
export function cachedPspLlvmShimBin(env: NodeJS.ProcessEnv = process.env): string {
  return join(pocketStackCacheRoot(env), "psp/llvm-shim/bin");
}

/* clang stamps `.MIPS.abiflags` with fp_abi=SINGLE for -msingle-float, while the
 * pinned SDK's libc.a and every rustc object declare fp_abi=DOUBLE. rust-lld
 * validates that pairing before it applies the link script, so the mixed link is
 * rejected even though hosts/psp/targets/mipsel-sony-psp.json discards the
 * section from the output. The wrapper clears it from C objects after each
 * compile, which drops metadata the link would have dropped anyway.
 *
 * Written per resolved LLVM so a toolchain change rewrites it, and skipped when
 * that LLVM ships no llvm-objcopy — there is nothing to clear the section with,
 * and the raw bin keeps today's behavior. */
function shimScript(realBin: string, tool: string): string {
  const real = join(realBin, tool);
  const objcopy = join(realBin, "llvm-objcopy");
  if (tool !== "clang" && tool !== "clang++") {
    return `#!/bin/sh\n# Generated by tools/psp-toolchain.ts — passthrough.\nexec "${real}" "$@"\n`;
  }
  return `#!/bin/sh
# Generated by tools/psp-toolchain.ts. Do not edit; regenerated per LLVM.
# Clears .MIPS.abiflags from compiled objects so rust-lld accepts the link.
"${real}" "$@" || exit $?
out=""; compiling=0; prev=""
for a in "$@"; do
  [ "$prev" = "-o" ] && out="$a"
  [ "$a" = "-c" ] && compiling=1
...
if [ "$compiling" = "1" ] && [ -n "$out" ] && [ -f "$out" ]; then
  "${objcopy}" --remove-section=.MIPS.abiflags "$out" 2>/dev/null || true
fi
exit 0
`;
}

export function ensurePspLlvmShim(
  llvmBin: string,
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  if (!existsSync(join(llvmBin, "llvm-objcopy"))) return undefined;
  const bin = cachedPspLlvmShimBin(env);
  mkdirSync(bin, { recursive: true });
  for (const tool of ["clang", "clang++", "llvm-ar"]) {
    if (!existsSync(join(llvmBin, tool))) continue;
    const path = join(bin, tool);
    const script = shimScript(llvmBin, tool);
  ...
  return existsSync(join(bin, "clang")) ? bin : undefined;
}
...
export function resolvePspBuildToolchain(env: NodeJS.ProcessEnv = process.env): PspBuildToolchain {
  ...
  return {
    ...
    environment: {
      ...
      PSP_SDK: sdk.path,
      PSPDEV: sdk.path,
      PATH: [cargoPspBin, ensurePspLlvmShim(llvmBin, env), llvmBin, join(home, ".cargo", "bin"), env.PATH]
        .filter(Boolean)
        .join(":"),
    },
  };
